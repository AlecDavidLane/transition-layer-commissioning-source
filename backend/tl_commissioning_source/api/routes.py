...
def list_patterns():
    return {
        "patterns": [
            {"key": p.key, "name": p.name, "description": p.description, "parameters": p.parameters}
...
async def stop_soak(request: Request):
    ...
    state.store.record_event("soak_stopped", "Soak stopped by operator", session_id=state.current_session_id())
...
def create_session(body: schemas.SessionCreate, request: Request):
    ...
    store = state.store
    try:
        project_id = store.upsert_project(body.project_name, body.client, body.site, body.engineer or state.config.get("engineer", "default_name") or "")
...
@api.post("/sessions/{session_id}/tests/{test_key}/attempts", tags=["sessions"], dependencies=[ControlRequired])
async def record_attempt(session_id: str, test_key: str, body: schemas.AttemptCreate, request: Request):
    state = _state(request)
    try:
        ...
        raise HTTPException(409, str(exc)) from exc
    await state.hub.publish("attempt", {"session_id": session_id, "test_key": test_key, "result": body.result})
    return attempt
...
def _resolve_current(state, include_completed: bool = False) -> str:
    row = state.db.query_one(
        "SELECT id FROM session WHERE status IN ('in_progress','review') AND deleted = 0 ORDER BY started_at DESC LIMIT 1"
    )
    if row is None and include_completed:
        row = state.db.query_one(
            "SELECT id FROM session WHERE status IN ('completed_passed','completed_failed') AND deleted = 0 ORDER BY completed_at DESC LIMIT 1"
        )
    if row is None:
        raise HTTPException(404, "No active session. Create one with POST /api/v1/sessions (use 'autostart': true to skip the draft stage).")
    return row["id"]
...
@api.post("/current-session/tests/{test_key}/attempts", tags=["current-session"], dependencies=[ControlRequired])
async def current_record_attempt(test_key: str, body: schemas.AttemptCreate, request: Request):
    state = _state(request)
    return await record_attempt(_resolve_current(state), test_key, body, request)


@api.post("/current-session/complete", tags=["current-session"], dependencies=[ControlRequired])
async def current_complete(request: Request):
    state = _state(request)
    return await complete_session(_resolve_current(state), request)


@api.post("/current-session/report", tags=["current-session"], dependencies=[ControlRequired])
async def current_report(request: Request):
    state = _state(request)
    return await create_report(_resolve_current(state, include_completed=True), request)


@api.post("/sessions/{session_id}/attempts/{attempt_id}/evidence", tags=["evidence"], dependencies=[ControlRequired])
async def upload_evidence(session_id: str, attempt_id: int, request: Request, file: UploadFile = File(...), caption: str = Form("")):
    state = _state(request)
    attempt = state.db.query_one("SELECT id FROM test_attempt WHERE id = ? AND session_id = ?", (attempt_id, session_id))
    if attempt is None:
    ...
    data = await file.read()
    try:
        stored = store_photo(data, file.content_type or "", state.config.evidence_dir, session_id, attempt_id, max_bytes)
    except EvidenceError as exc:
        raise HTTPException(422, str(exc)) from exc
    cursor = state.db.execute(
        "INSERT INTO evidence (attempt_id, original_path, derivative_path, caption, mime_type, sha256) VALUES (?,?,?,?,?,?)",
        (attempt_id, stored["original_path"], stored["derivative_path"], caption[:500], stored["mime_type"], stored["sha256"]),
    )
    return {"id": cursor.lastrowid, **stored, "caption": caption[:500]}


@api.put("/evidence/{evidence_id}/caption", tags=["evidence"], dependencies=[ControlRequired])
def update_caption(evidence_id: int, body: schemas.CaptionUpdate, request: Request):
    ...
    state.db.execute("UPDATE evidence SET caption = ? WHERE id = ?", (body.caption, evidence_id))
    return {"ok": True}
...
def list_reports(session_id: str, request: Request):
    rows = _state(request).db.query("SELECT * FROM report WHERE session_id = ? ORDER BY revision DESC", (session_id,))
...
def recent_reports(request: Request):
    rows = _state(request).db.query(
        "SELECT r.*, s.room, p.name AS project_name FROM report r JOIN session s ON s.id = r.session_id JOIN project p ON p.id = s.project_id ORDER BY r.generated_at DESC LIMIT 50"
    )
    return {"reports": [dict(r) for r in rows]}
...
async def events_ws(websocket: WebSocket):
    ...
    try:
        await websocket.send_json({"type": "hello", "payload": state.pattern_state()})
        while True:
